...
pub async fn index(
    ...
    let assets: Vec<AssetRow> = asset_rows
        ...
        .map(|(id, symbol, name, type_code, chain_code, risk_code, coingecko_id, yahoo_ticker, target_pct, is_stable, active)| AssetRow {
            id, symbol,
            ...
            target_pct: target_pct.unwrap_or(0.0),
            is_stable, active,
        })
        .collect();
    ...
    // Accounts — all
    let acct_rows: Vec<(i64, String, String, Option<String>, Option<String>, i64, Option<String>)> = sqlx::query_as(
    ...
    let accounts: Vec<AccountRow> = acct_rows
        .into_iter()
        .map(|(id, name, type_code, institution, chain_code, active, notes)| AccountRow {
            id, name, type_code,
